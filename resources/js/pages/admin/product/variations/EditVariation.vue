...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Edit Variation', href: `/admin/products/${props.product.id}/variations/${props.variation.id}/edit` },
];

// Initialize form with variation data
onMounted(() => {
    form.display_name = props.variation.display_name;
    form.attribute_values = props.variation.attribute_values;
    form.price = props.variation.price;
    form.sale_price = props.variation.sale_price || null;
    form.stock_quantity = props.variation.stock_quantity;
    form.sku = props.variation.sku || '';
    form.is_active = props.variation.is_active;
});
...
const submit = () => {
    ...
    if (!form.display_name.trim()) {
        toast.error('Display name is required', 'Validation Error');
        isLoading.value = false;
        return;
    }
    
    if (!form.attribute_values.trim()) {
        toast.error('Attribute values are required', 'Validation Error');
        isLoading.value = false;
        return;
    }
    
    if (form.price <= 0) {
        toast.error('Price must be greater than 0', 'Validation Error');
        isLoading.value = false;
        return;
    }
    
    if (form.sale_price && form.sale_price >= form.price) {
        toast.error('Sale price must be less than regular price', 'Validation Error');
        isLoading.value = false;
        return;
    }
    
    if (form.stock_quantity < 0) {
        toast.error('Stock quantity cannot be negative', 'Validation Error');
        isLoading.value = false;
        return;
    }

    form.put(`/admin/products/${props.product.id}/variations/${props.variation.id}`, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
            isLoading.value = false;
        ...
        onError: (errors) => {
            ...
            if (Object.keys(errors).length > 0) {
                ...
                toast.error('Failed to update variation. Please check the form and try again.', 'Error');
            }
        },
        onFinish: () => {
            isLoading.value = false;
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 overflow-x-auto rounded-xl p-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Edit Variation</h1>
                    <p class="text-gray-600 dark:text-gray-400">
                        Editing variation for: <span class="font-medium">{{ product.name }}</span>
                    </p>
                </div>
                <Link
            ...
            <!-- Form -->
            <div class="relative rounded-lg bg-white p-6 shadow-sm dark:bg-gray-800">
                <!-- Loading Overlay -->
                <div 
                ...
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div class="flex justify-end gap-4 border-t border-gray-200 pt-6 dark:border-gray-700">
                        ...
                        >
                            <Save class="h-4 w-4" />
                            {{ form.processing || isLoading ? 'Updating...' : 'Update Variation' }}
                        </button>
                    </div>
                </form>
            </div>
